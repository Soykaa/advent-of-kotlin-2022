fun main() {
    fun parseStacks(processedInput: MutableList<List<String>>): List<MutableList<Char>> {
    ...
    fun transformInput(input: List<String>): Pair<MutableList<List<String>>, MutableList<String>> {
...
//        println(cmds.size)
    ...
    fun getTopElements(parsedStacks: List<List<Char>>): String =
        parsedStacks.map { it.lastOrNull() ?: "" }.joinToString("")

    fun processCommands(input: List<String>, move: (List<MutableList<Char>>, Int, Int, Int) -> Unit): String {
    ...
    fun move1(stacks: List<MutableList<Char>>, amount: Int, from: Int, to: Int) = repeat(amount) {
        stacks[to].add(stacks[from].removeLast())
    }

    fun move2(stacks: List<MutableList<Char>>, amount: Int, from: Int, to: Int) {
