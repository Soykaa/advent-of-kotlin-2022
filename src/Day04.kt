...
fun main() {
    fun IntRange.isSubset(other: IntRange): Boolean = first in other && last in other

    fun IntRange.isOverlap(other: IntRange): Boolean = first in other || last in other

    fun getRanges(input: List<String>): List<List<IntRange>> = input.map { line ->
        line.split(",")
            .map {
                it.split("-")
