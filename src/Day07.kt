...
fun main() {
    fun createFileOrDir(parameter: String, parent: Node?): Node = if (parameter == "dir")
        Node(true, 0, parent)
    else {
        Node(false, parameter.toInt(), parent)
    }

    fun processCd(cdArg: String, currentNode: Node): Node = when (cdArg) {
        "/" -> root
        ".." -> currentNode.parent ?: currentNode
        else -> currentNode.children[cdArg]!!
    }

    fun processFilesAndDirs(arguments: List<String>, currentNode: Node) {
        currentNode.children.putIfAbsent(arguments[1], createFileOrDir(arguments[0], currentNode))
    }

    fun calcSize(currentNode: Node): Int {
